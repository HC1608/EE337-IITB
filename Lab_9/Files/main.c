...
void main(void)
{
  ...
  lcd_init();
	...
	while(1)
	{
		...
		x = adc(3);																					//Read analog value from 3th channel of ADC Ic MCP3008
		
		//Use appropriate multiplier instead of 0.1 for converting it to degree Celsius
		
		temperature = (x*3.2258*0.1); 											//NOTE: LM35 O/P: 10mV/°C, Refer to LM35 datasheet for more information
