...
{
  	/*
			Configure SPCON is not a bit addressable sfr
																												
			(1) Configure MSTR to set uC in Master mode MSTR = 1 [SPCON.4=1]
			(2) Configure SSDIS to disable SSBAR thus [SPCON.5=0]
					P1.1(SSBAR) will be available as standard I/O pin 	             	 	 
			(3) Configure clock polarity s.t. sck from uC set to 1 in idle state [SPCON.3=1]
					Refer to SPCON registers table no 75 from AT89C5131 datasheet
			(4) Configure clock phase i.e. CPHA: Refer SPI timing  CPHA - [SPCON.2=1]
			...
			(5) Fclk/16 & Fclk Periph=12MHz ,thus BAUD RATE=750KHz [SPR2(SPCON.7)=0, SPR0(SPCON.0)=1, SPR1(SPCON.1)=1]
			(6) Enable SPI interrupts (i.e. configure IEN1 and EA)
			(7) Enable SPI module (i.e. configure SPCON )
	*/

	
	SPCON = 0x3F;																																																								
	IEN1 |= 0x04;	/*ESPI = 1, IEN1.2 = 1*/									//enable spi interrupts
	EA = 1;																									//enable interrupts 
	SPCON |= 0x40;/*SPEN = 1 (SPCON.6=1)*/									//Enable spi module 
}
